...
#define XSM3_printf(...) \
    do { \
        if (hxsm3->printf_cb != NULL) { \
            hxsm3->printf_cb(__VA_ARGS__); \
        } \
    } while (0)

static const uint8_t xsm3_id_data_ms_controller[0x1D] = {
...
void xsm3_set_identification_data(xsm3_handle_t* hxsm3) {
...
void xsm3_do_challenge_init(xsm3_handle_t* hxsm3, const uint8_t challenge_packet[0x22]) {
    ...
    XSM3_printf("  [ Encrypting random console data ]\n");
    ...
    XSM3_printf("  [ Generating random controller data ]\n");
    // srand(time(NULL));
    for (i = 0; i < 0x10; i++) {
    ...
    XSM3_printf("  [ Calculating challenge init hash ]\n");
    ...
    XSM3_printf("  [ Encrypting challenge response ]\n");
    ...
    XSM3_printf("  [ Calculating MAC for challenge response ]\n");
    ...
    XSM3_printf("  [ Calculating ACR for challenge response ]\n");
    ...
    XSM3_printf("  [ Calculating checksum for challenge response ]\n");
    ...
    XSM3_printf("  [ Modifying console random data post-challenge init ]\n");
...
void xsm3_do_challenge_verify(xsm3_handle_t* hxsm3, const uint8_t challenge_packet[0x16]) {
    uint8_t incoming_packet_mac[0x8];
    ...
    XSM3_printf("  [ Clearing challenge verify response buffers ]\n");
    // clear response buffers
    memset(hxsm3->challenge_response, 0, sizeof(hxsm3->challenge_response));
    memset(hxsm3->decrypt_buffer, 0, sizeof(hxsm3->decrypt_buffer));
    // set header and packet length of challenge response
    ...
    XSM3_printf("  [ Calculating MAC for challenge verify response ]\n");
    ...
    // calculate the checksum for the response packet
    XSM3_printf("  [ Calculating checksum for challenge verify response ]\n");
...
bool xsm3_init(xsm3_handle_t* hxsm3, xsm3_get_rand_cb_t get_rand_cb, xsm3_printf_cb_t printf_cb, uint16_t vid, uint16_t pid, const uint8_t serial[20]) {
    if (hxsm3 == NULL) {
        XSM3_printf("[ xsm3_init: Invalid parameters ]\n");
        return false;
    }
    ...
    hxsm3->get_rand_cb = get_rand_cb;
    hxsm3->printf_cb = printf_cb;
    memcpy(hxsm3->ctrl_id_data, xsm3_id_data_ms_controller, sizeof(xsm3_id_data_ms_controller));
    if (serial != NULL) {
        memcpy(hxsm3->ctrl_id_data + 6, serial, 20);
    }
    hxsm3->ctrl_id_data[6] = hxsm3->get_rand_cb() & 0xFF;
    uint8_t* id_data = hxsm3->ctrl_id_data;
    id_data += 5; 
    if (vid != 0) {
        memcpy(id_data + 15, &vid, sizeof(uint16_t));
    }
    if (pid != 0) {
        memcpy(id_data + 17, &pid, sizeof(uint16_t));
    }
    hxsm3->ctrl_id_data[0x1C] = xsm3_calculate_checksum(hxsm3->ctrl_id_data);
    hxsm3->state = 0x0001;
    xsm3_set_identification_data(hxsm3);
    return true;
}
...
bool xsm3_get_response(xsm3_handle_t* hxsm3, uint8_t bmRequestType, uint8_t bRequest, uint8_t** resp, uint16_t* resp_len) {
    if ((bmRequestType != 0xC1) || (hxsm3 == NULL) || (resp_len == NULL) || (!hxsm3->state)) {
        XSM3_printf("[ xsm3_get_response: Invalid parameters ]\n");
        return false;
    }
    switch (bRequest) {
    case 0x81:
        XSM3_printf("[ xsm3_get_response: Providing identification data ]\n");
    ...
    //     return true;
    case 0x83:
        XSM3_printf("[ xsm3_get_response: Providing challenge response ]\n");
        *resp = hxsm3->challenge_response;
        *resp_len = sizeof(hxsm3->challenge_response);
        return true;
    case 0x86:
        XSM3_printf("[ xsm3_get_response: Providing auth state response ]\n");
        hxsm3->state = 2;
        *resp = (uint8_t*)&hxsm3->state;
        *resp_len = sizeof(hxsm3->state);
        return true;
    default:
        XSM3_printf("[ xsm3_get_response: Unknown bRequest 0x%02X ]\n", bRequest);
        return false;
    }
}

bool xsm3_set_request(xsm3_handle_t* hxsm3, uint8_t bmRequestType, uint8_t bRequest, const uint8_t* req_data) {
    if ((bmRequestType != 0x41) || (hxsm3 == NULL) || (req_data == NULL) || (!hxsm3->state)) {
        XSM3_printf("[ xsm3_set_request: Invalid parameters ]\n");
        return false;
    }
    switch (bRequest) {
    case 0x81:
        return true;
    case 0x82:
        XSM3_printf("[ xsm3_set_request: Processing challenge init request ]\n");
        xsm3_do_challenge_init(hxsm3, req_data);
        return true;
    case 0x87:
        XSM3_printf("[ xsm3_set_request: Processing challenge verify request ]\n");
        xsm3_do_challenge_verify(hxsm3, req_data);
        return true;
    case 0x84:
    case 0x83:
        return true;
    default:
        XSM3_printf("[ xsm3_set_request: Unknown bRequest 0x%02X ]\n", bRequest);
        return false;
    }
}